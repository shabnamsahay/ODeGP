...
modelSelection <- function(statistic, threshold) {
  ...
  cat("If k trajectories from the same dataset are being analyzed simultaneously\n,
      a Bonferroni-type multiple-hypothesis correction can be applied\n
      by multiplying this Bayes factor with (1-0.5^(1/k))/(0.5^(1/k)).\n")
